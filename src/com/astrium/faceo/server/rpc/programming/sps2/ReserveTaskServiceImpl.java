...
/**
 * Server side implementation of the ReserveTaskService interface.
 * 
 ...
 *
 */
public class ReserveTaskServiceImpl extends RemoteServiceServlet implements ReserveTaskService {

	/**
	 * 
	 */
	private static final long serialVersionUID = 4980955068171494376L;

	/** Le logger de cette classe */
	private static Logger log = Logger.getLogger(ReserveTaskServiceImpl.class);

	String INFOS_CLASSE = "ReserveTaskServiceImpl";
	...
	public TaskingResponseBean getReserveResponse(
		...
		if (_operation == null){
			...
			if (!_operation.equalsIgnoreCase(Sps2GeneralConstants.SPS_RESERVE)){
		...
		try {
			...
			if (_webAccessMode) {
				if (simulationProgramming.equalsIgnoreCase(Sps2GeneralConstants.FALSE)) {
					...
					int pos = requestURL.lastIndexOf(Sps2GeneralConstants.RESERVE_CONTROLLER);
					String serverURL = requestURL;
					if (pos >0) {
						serverURL = requestURL.substring(0, pos);
					}

					log.debug("\n" + INFOS_CLASSE + " : facadeProgramming.getReserveResponse");  
					response = 
						facadeProgramming.getReserveResponse(_parameters, serverURL);
				} else {
			...
			log.debug("\n ------------------------------------------------------------------------------");    
			log.debug("\n" + INFOS_CLASSE + " : getReserveResponse");    
			...
			log.info("\n" + INFOS_CLASSE + " : getReserveResponse : FacadeSps2Exception : " + exception.getMessage());
			return response;
