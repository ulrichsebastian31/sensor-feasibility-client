...
/**
 * Server side implementation of the UpdateTaskService interface.
 * 
 ...
 *
 */
public class UpdateTaskServiceImpl extends RemoteServiceServlet implements UpdateTaskService {

	/**
	 * 
	 */
	private static final long serialVersionUID = -1702877844568503204L;

	/** Le logger de cette classe */
	private static Logger log = Logger.getLogger(UpdateTaskServiceImpl.class);

	String INFOS_CLASSE = "UpdateTaskServiceImpl";
	 ...
	 * @param _parameters (ProgrammingRequestBean)	: 'Update' parameters
	...
	public TaskingResponseBean getUpdateResponse(
		...
		if (_operation == null){
			...
			if (!_operation.equalsIgnoreCase(Sps2GeneralConstants.SPS_UPDATE)){
		...
		try {
			...
			if (_webAccessMode) {
				if (simulationProgramming.equalsIgnoreCase(Sps2GeneralConstants.FALSE)) {
					...
					int pos = requestURL.lastIndexOf(Sps2GeneralConstants.UPDATE_TASK_CONTROLLER);
					String serverURL = requestURL;
					if (pos >0) {
						serverURL = requestURL.substring(0, pos);
					}

					log.debug("\n" + INFOS_CLASSE + " : facadeTaskRequests.getUpdateResponse");  
					FacadeTaskRequests facadeTaskRequests = new FacadeTaskRequests();
					response = 
						facadeTaskRequests.getUpdateResponse(_parameters, serverURL);
				} else {
			...
			log.debug("\n ------------------------------------------------------------------------------");    
			log.debug("\n" + INFOS_CLASSE + " : getUpdateResponse");    
			...
			log.info("\n" + INFOS_CLASSE + " : getUpdateResponse : FacadeSps2Exception : " + exception.getMessage());
			return response;
